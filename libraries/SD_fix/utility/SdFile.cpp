...
String SdFile::ls_String(uint8_t flags, uint8_t indent) {
	String buf = "";
	dir_t* p;
	...
	while ((p = readDirCache())) {
		...
		// print file name with possible blank fill
		buf += printDirName_String(*p, flags & (LS_DATE | LS_SIZE) ? 14 : 0);
		buf += '\n';
		// print modify date/time if requested
		if (flags & LS_DATE) {
			printFatDate(p->lastWriteDate);
...
String SdFile::printDirName_String(const dir_t& dir, uint8_t width) {
	String buf = "";
  uint8_t w = 0;
  for (uint8_t i = 0; i < 11; i++) {
    if (dir.name[i] == ' ')continue;
